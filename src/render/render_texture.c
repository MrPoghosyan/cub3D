...
static void	init_tex_step(t_ray *ray, t_tex_img *tex, double *step,
		double *tex_pos, int h)
{
	*step = 1.0 * tex->height / ray->line_h;
	*tex_pos = (ray->draw_start - h / 2 + ray->line_h / 2) * (*step);
}

static void	draw_wall_column(t_cub *cub, t_ray *ray, t_tex_img *tex, int x)
{
	double	tex_pos;
	double	step;
	double	shade;
